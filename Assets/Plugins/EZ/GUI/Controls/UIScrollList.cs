...
public class UIScrollList : MonoBehaviour, IUIObject
{
	...
	public void SetViewableAreaPixelDimensions(Camera cam, int width, int height)
	{
		...
		viewableArea = new Vector2(width * worldUnitsPerScreenPixel, height * worldUnitsPerScreenPixel);
			...
			if (orientation == ORIENTATION.HORIZONTAL)
			{
				...
				else
				{
					...
					if (direction == DIRECTION.TtoB_LtoR)
						...
						x = (viewableArea.x * 0.5f) - item.BottomRightEdge.x - ((spacingAtEnds) ? (itemSpacing) : (0));
				}
			...
			else
			{
				...
				else
				{
					...
					if(direction == DIRECTION.TtoB_LtoR)
						...
						y = (viewableArea.y * -0.5f) - item.BottomRightEdge.y + ((spacingAtEnds) ? (itemSpacing) : (0));
				}
